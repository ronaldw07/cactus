...
export function resolveCompactionSettings(settings: CompactionSettings, contextWindow: number): CompactionSettings {
	...
	const reserveTokens = Math.max(256, Math.min(settings.reserveTokens, Math.floor(contextWindow * 0.3)));
	const keepRecentTokens = Math.max(256, Math.min(settings.keepRecentTokens, Math.floor(contextWindow * 0.4)));
